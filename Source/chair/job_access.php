...
<?php if (isset($_POST['access_update'])) : ?>
    <?php
    ...
    $result4 = $conn->query("SELECT * FROM `members` ");

    while ($row = $result4->fetch_array(MYSQLI_ASSOC)) {
        // echo $row["name"];
        $member_id = $row["id"];
        $member_id_str = "member-" . $member_id;
        if (isset($_POST[$member_id_str])) {
            $result5 = $conn->query("INSERT INTO `review_access`(`job_id`, `member_id`) VALUES ('$job_id','$member_id')");
        }
    }
    echo '<script>alert("Members Updated Successfully")</script>';
            ...
            <div class="page-content">
                <div class="page-bar">
                    <div class="page-title-breadcrumb">
                        <div class=" pull-left">
                            <div class="page-title">Job Review Access</div>
                        </div>
                        <ol class="breadcrumb page-breadcrumb pull-right">
                            ...
                            <li class="active">Job Review Access</li>
                        </ol>
                    </div>
                </div>
                <div class="row">
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <div class="card card-box">
                                                            <div class="card-head">
                                                                <header>Allow Access To Members</header>
                                                                <div class="tools">
                                                                ...
                                                                <form action="" method="post">
                                                                    <table class="table table-striped table-bordered table-hover table-checkable order-column valign-middle" id="example4">
                                                                        ...
                                                                        <tbody>
                                                                            ...
                                                                            $result = $conn->query("SELECT * FROM `members` WHERE dept_id = '$dept_id'");

                                                                            while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                                                                                // echo $row["name"];
                                                                                $member_id = $row["id"];
                                                                                $result2 = $conn->query("SELECT COUNT(job_id) as count FROM review_access WHERE job_id = $job_id && member_id = $member_id;");
                                                                                $count = $result2->fetch_array(MYSQLI_ASSOC);
                                                                                ...
                                                                                <td class="left">' . $row["joining_date"] . '</td>
																				
																				<td><a href="mailto:' . $row["email"] . '">
																				' . $row["email"] . ' </a></td>
																				<td>
                                                                                
                                                                                <input name="member-' . $member_id . '" type="checkbox" ' . $checked . '>
                                                                    ...
                                                                    <input name="access_update" type="hidden" value="1">
